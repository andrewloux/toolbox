...
async function scrapeQuizExplanations() {
  ...
  try {
    ...
    await page.goto(
      "https://michbrite.michener.ca/d2l/le/lessons/7298/units/47008",
      {
    ...
    const popupItems = await frame.evaluate(() => {
      ...
      const mainItems = document.querySelectorAll('.navigation-menu .navigation-item');
      log(`Found ${mainItems.length} main navigation items`);

      mainItems.forEach((mainItem, mainIndex) => {
        const mainTitle = mainItem.querySelector('.title-text span')?.textContent.trim() || '';
        ...
        nestedItems.forEach((nested, nestedIndex) => {
          const nestedTitle = nested.querySelector('.title-text span')?.textContent.trim() || '';
    ...
    for (let i = 0; i < popupItems.length; i++) {
      ...
      try {
        ...
        await frame.waitForSelector(triangleSelector, { visible: true, timeout: 5000 });
        ...
        const popupState = await frame.evaluate((selector, expectedText) => {
        ...
        }, popupSelector, item.popupText);
        console.log(`  Pop-up element state:`, popupState);
        
        if (!popupState.matches) {
          throw new Error(`Found wrong element. Expected "${item.popupText}" but found "${popupState.text}"`);
        }
        
        await frame.waitForSelector(popupSelector, { visible: true, timeout: 5000 });
        ...
        await frame.waitForSelector('.resizing-iframe-container', { timeout: 5000 });
        await frame.waitForSelector('d2l-iframe-wrapper-for-react', { timeout: 5000 });
        ...
        const content = await frame.evaluate(async () => {
          ...
          const wrapper = await waitFor('d2l-iframe-wrapper-for-react', document);
          ...
          const iframe = await waitFor('iframe.resizing-iframe.html-topic-iframe', shadow);
        ...
        if (!content || !content.trim()) {
          throw new Error("Could not find any meaningful content in the iframe");
        }
