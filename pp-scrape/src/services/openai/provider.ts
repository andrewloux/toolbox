...
export class DefaultOpenAIProvider implements OpenAIProvider {
    ...
    async analyze(jobDescription: string, resumeText: string): Promise<OpenAIResponse> {
        try {
...
5. Cultural fit indicators`
                    ...
                    tool_choice: { type: "function", function: { name: "analyze_job_fit" } },
                    temperature: 0.3
                })
            });
            ...
            if (toolCall?.function) {
                try {
                    ...
                    Logger.error(SERVICE_NAME, 'Failed to parse analysis result', { error });
                    throw error;
                }
            } else {
